...
-- ─── HELPER: is_admin() ─────────────────────────────────────
-- Security definer bypasses RLS on profiles, preventing recursion
...
-- ─── SERVICE ACCOUNTS ───────────────────────────────────────
-- One per customer — their pond / billing account
create table service_accounts (
  ...
  pond_notes          text,
  stripe_customer_id  text,  -- set after Stripe onboarding
...
create table service_visits (
  ...
  notes               text,   -- admin/scheduler notes (visible to customer)
  tech_notes          text,   -- notes added by tech after visit (internal)
  created_at          timestamptz default now()
);
...
-- ─── BASKET ITEMS ───────────────────────────────────────────
-- Products requested for a visit (by customer or billed by tech on-site)
...
-- ─── REALTIME ───────────────────────────────────────────────
-- Enable realtime for messaging and visit status updates
